          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wide text-muted">
                ...
                <th className="font-medium text-right">Trail stop</th>
            ...
            <tbody className="num">
              {pos.map((p) => {
                const up = (p.uPnl ?? 0) >= 0;
                const trailing = p.strategy === 'trend';
                return (
                  <tr key={p.symbol} className="border-t border-edge/60">
                    <td className="py-2.5 display font-semibold">
                      {p.symbol.replace('USDT', '')}
                      {trailing && <span className="ml-1.5 align-middle text-[9px] uppercase tracking-wide text-brand">trend</span>}
                    </td>
                    ...
                    <td className="text-right text-warn">{p.stopPrice ? `$${price(p.stopPrice)}` : '—'}</td>
